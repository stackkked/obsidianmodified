...
local Library = {
    ...
    Notifications = {},
    NotifySide = "Right",
    NotifyPosition = "TopRight", -- TopLeft, TopMiddle, TopRight, CenterLeft, CenterRight, BottomLeft, BottomMiddle, BottomRight
    ...
    Scheme = {
        ...
        SuccessColor = Color3.fromRGB(45, 204, 112),
        WarningColor = Color3.fromRGB(255, 184, 48),
        ErrorColor = Color3.fromRGB(255, 76, 76),
...
--// Notification \\--
-- Each position has its own container (anchored to the matching screen edge/corner),
-- its own stacking order, and knows which axis+direction new notifications should
-- slide in/out from.
--   Grow = "Down"  -> stack grows downward from the top edge (top-anchored & center positions)
--   Grow = "Up"    -> stack grows upward from the bottom edge (bottom-anchored positions)
--   EnterAxis      -> "X" (slides in horizontally) or "Y" (slides in vertically)
--   EnterFrom      -> off-screen starting UDim2 for the Holder's slide-in tween
local NotifyPositionConfig = {
    TopLeft = {
        AnchorPoint = Vector2.new(0, 0),
        Position = UDim2.fromOffset(6, 6),
        Grow = "Down",
        EnterAxis = "X",
        EnterFrom = UDim2.new(-1, -8, 0, -2),
    },
    TopMiddle = {
...
local NotifyOrder = {}
do
    for PositionName, Config in NotifyPositionConfig do
...
function Library:UpdateNotificationPositions(Snap: boolean?)
    for PositionName, Order in NotifyOrder do
        local Config = NotifyPositionConfig[PositionName]
        local Area = NotificationAreas[PositionName]
        if not (Config and Area) then continue end

        if Config.Grow == "Up" then
            local RunningY = Area.AbsoluteSize.Y

            for _, FakeBackground in Order do
                local Data = Library.Notifications[FakeBackground]
                if not (Data and FakeBackground.Parent) then continue end

                RunningY = RunningY - FakeBackground.AbsoluteSize.Y

                local Target = UDim2.new(Config.AnchorPoint.X, 0, 0, RunningY)
...
function Library:SetNotifyPosition(Position: string)
    if not NotifyPositionConfig[Position] then
        warn(string.format("Unknown notify position %q, keeping %q", tostring(Position), Library.NotifyPosition))
        return
    end

    Library.NotifyPosition = Position
    Library:UpdateNotificationPositions(true)
end

-- Kept for backwards compatibility with the old Left/Right-only system.
-- Maps onto the new position system (both stay top-anchored, same as before).
function Library:SetNotifySide(Side: string)
    Library.NotifySide = Side
    Library:SetNotifyPosition(Side:lower() == "left" and "TopLeft" or "TopRight")
end

--// Notification Type Presets \\--
-- "Custom" is intentionally not listed here: for Type == "Custom" the icon/color
-- are never auto-assigned (you set your own via Icon/IconColor), but the sound
-- DOES get a default — it falls back to the Success sound (see below).
--
-- IMPORTANT: SoundId below are PLACEHOLDERS (0 = "no sound", nothing will play).
-- I can't verify live Roblox catalog audio ids from here, so I won't invent
-- numbers and pass them off as working sounds. Put your own ids in:
--   - grab a free one from the Studio Toolbox ("Audio" tab, search "notification"/"ding"/"error")
--   - or upload your own .mp3/.ogg on the Creator Dashboard and use its asset id
-- Then just replace the 0s below with rbxassetid numbers (e.g. 123456789).
local NotifyTypePresets = {
    -- SoundId 0 means silent. Default sounds chosen to match notification tone:
    -- Users can override per-notification via { SoundId = rbxassetid://... }
    Success = { Icon = "circle-check", IconColor = "SuccessColor", SoundId = 139746569667955 }, -- Fears to Fathom notification
    Warning = { Icon = "triangle-alert", IconColor = "WarningColor", SoundId = 139746569667955 }, -- Fears to Fathom notification
    Error   = { Icon = "circle-x",     IconColor = "ErrorColor",   SoundId = 139746569667955 }, -- Fears to Fathom notification
}
...
function Library:Notify(...)
    ...
    if typeof(Info) == "table" then
        ...
        -- Data.Position: TopLeft, TopMiddle, TopRight, CenterLeft, CenterRight,
        -- BottomLeft, BottomMiddle, BottomRight. Defaults to Library.NotifyPosition.
        Data.Position = Info.Position or Library.NotifyPosition
        if not NotifyPositionConfig[Data.Position] then
            warn(string.format("Unknown notify Position %q, falling back to %q", tostring(Data.Position), Library.NotifyPosition))
            Data.Position = Library.NotifyPosition
        end

        -- Data.Type: "Success" | "Warning" | "Error" | "Custom" | nil
        Data.Type = Info.Type
        Data.Icon = Info.Icon
        Data.BigIcon = Info.BigIcon
        Data.IconColor = Info.IconColor

        local TypePreset = Data.Type and NotifyTypePresets[Data.Type]
        if TypePreset then
            Data.Icon = Data.Icon or TypePreset.Icon
            Data.IconColor = Data.IconColor or TypePreset.IconColor

            if not Data.SoundId or Data.SoundId == 0 then
                Data.SoundId = TypePreset.SoundId
            end
        elseif Data.Type == "Custom" then
            -- No icon/color default for Custom, but default the sound to Success's.
            if not Data.SoundId or Data.SoundId == 0 then
                Data.SoundId = NotifyTypePresets.Success.SoundId
            end
        end

        if Data.SoundId == 0 then
            Data.SoundId = nil -- 0 means "not set" -- avoid trying to play/stream asset id 0
        end
    ...
    local PositionConfig = NotifyPositionConfig[Data.Position]
    local Area = NotificationAreas[Data.Position]

    local FakeBackground = New("Frame", {
        AnchorPoint = Vector2.new(PositionConfig.AnchorPoint.X, 0),
        AutomaticSize = Enum.AutomaticSize.Y,
        BackgroundTransparency = 1,
        Position = UDim2.new(PositionConfig.AnchorPoint.X, 0, 0, 0),
        Size = UDim2.fromScale(1, 0),
        Visible = false,
        Parent = Area,
    })
    ...
    local Holder = New("Frame", {
        AutomaticSize = Enum.AutomaticSize.Y,
        BackgroundColor3 = "MainColor",
        Position = PositionConfig.EnterFrom,
        Size = UDim2.fromScale(1, 1),
        ZIndex = 5,
    ...
    if Data.Title or Data.Icon then
    ...
    function Data:Resize()
        ...
        if Title then
            local X, Y =
                Library:GetTextBounds(Title.Text, Title.FontFace, Title.TextSize, (Area.AbsoluteSize.X / Library.DPIScale) - 24 - ExtraWidth - IconWidth)
        ...
        elseif IconLabel then
            -- No title text, but there's a (type-based or custom) icon: size the
            -- container to just fit the icon so it still renders correctly.
            TitleX = IconWidth
            TitleContainer.Size = UDim2.fromOffset(IconWidth, 16)
        end

        if Desc then
            local X, Y =
                Library:GetTextBounds(Desc.Text, Desc.FontFace, Desc.TextSize, (Area.AbsoluteSize.X / Library.DPIScale) - 24 - ExtraWidth)
    ...
    function Data:Destroy()
        ...
        if FakeBackground then
            local Idx = table.find(NotifyOrder[Data.Position], FakeBackground)
            if Idx then
                table.remove(NotifyOrder[Data.Position], Idx)
            end
        end
    ...
    if Data.SoundId then
        ...
        task.spawn(function()
            ...
            NotifSound.Volume = tonumber(Data.Volume) or 1
            NotifSound.Parent = SoundService
            NotifSound:Play()
            task.delay(NotifSound.TimeLength > 0 and NotifSound.TimeLength or 3, function()
                if NotifSound then NotifSound:Destroy() end
            end)
        end)
    end
    ...
    table.insert(NotifyOrder[Data.Position], FakeBackground)
...
local AdminPresenceData = {
    Admins = {},
    CurrentIndex = 1,
    Widget = nil,
    Collapsed = false,
    ChatLogWidget = nil,
    ChatLogsContainer = nil,
    Config = nil,
    Connections = {},
    Spectating = false,
    UI = {}
...
function AdminPresenceData.UpdateChatLogsUI()
    if not AdminPresenceData.ChatLogsContainer then return end
    local adminInfo = AdminPresenceData.Admins[AdminPresenceData.CurrentIndex]
    
    AdminPresenceData.UI.ChatLogsTitle.Text = adminInfo and (adminInfo.Player.Name .. "'s Chat Logs") or "Chat Logs"
    
    for _, child in ipairs(AdminPresenceData.ChatLogsContainer:GetChildren()) do
        if child:IsA("TextLabel") then child:Destroy() end
    end
    
    if adminInfo then
        for i, log in ipairs(adminInfo.Logs) do
            ...
            lbl.Parent = AdminPresenceData.ChatLogsContainer
        end
    end
    
    local listLayout = AdminPresenceData.ChatLogsContainer:FindFirstChildWhichIsA("UIListLayout")
    if listLayout then
        AdminPresenceData.ChatLogsContainer.CanvasSize = UDim2.new(0, 0, 0, listLayout.AbsoluteContentSize.Y)
        AdminPresenceData.ChatLogsContainer.CanvasPosition = Vector2.new(0, AdminPresenceData.ChatLogsContainer.CanvasSize.Y.Offset)
    end
end

function AdminPresenceData.UpdateUI()
    if #AdminPresenceData.Admins == 0 then
        if AdminPresenceData.Widget then
            TweenService:Create(AdminPresenceData.Widget, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Position = UDim2.new(0.5, -150, 0, -200)
            }):Play()
            task.delay(0.5, function() AdminPresenceData.Widget.Visible = false end)
        end
        return
    end

    if AdminPresenceData.Widget and not AdminPresenceData.Widget.Visible then
        AdminPresenceData.Widget.Visible = true
        AdminPresenceData.Widget.Position = UDim2.new(0.5, -150, 0, -200)
        TweenService:Create(AdminPresenceData.Widget, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Position = UDim2.new(0.5, -150, 0, 20)
        }):Play()
    end

    if AdminPresenceData.CurrentIndex > #AdminPresenceData.Admins then AdminPresenceData.CurrentIndex = #AdminPresenceData.Admins end
    if AdminPresenceData.CurrentIndex < 1 then AdminPresenceData.CurrentIndex = 1 end

    local adminInfo = AdminPresenceData.Admins[AdminPresenceData.CurrentIndex]
    
    if AdminPresenceData.UI.SwitcherLabel then
        AdminPresenceData.UI.SwitcherLabel.Text = tostring(AdminPresenceData.CurrentIndex) .. "/" .. tostring(#AdminPresenceData.Admins)
    end
    
    if AdminPresenceData.UI.Name then
        AdminPresenceData.UI.Name.Text = string.format("(%s) %s", adminInfo.Player.DisplayName, adminInfo.Player.Name)
    end
    
    if AdminPresenceData.UI.Role then
        AdminPresenceData.UI.Role.Text = adminInfo.Role
    end
    
    if AdminPresenceData.UI.JoinTime then
        AdminPresenceData.UI.JoinTime.Text = "Joined: " .. adminInfo.JoinTime
    end
    
    if AdminPresenceData.UI.LastAction then
        AdminPresenceData.UI.LastAction.Text = "Last Action: " .. adminInfo.LastAction
    end
    
    if AdminPresenceData.UI.Team then
        local teamName = adminInfo.Player.Team and adminInfo.Player.Team.Name or "—"
        AdminPresenceData.UI.Team.Text = "Team: " .. teamName
    end
    
    if AdminPresenceData.UI.Avatar then
        task.spawn(function()
            local content, isReady = Players:GetUserThumbnailAsync(adminInfo.Player.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420)
            if isReady then
                AdminPresenceData.UI.Avatar.Image = content
            end
        end)
    end
    
    if AdminPresenceData.UI.TeleportBtn then
        AdminPresenceData.UI.TeleportBtn.Text = "Teleport"
        AdminPresenceData.UI.TeleportBtn.BackgroundColor3 = Library.Scheme.MainColor
    end
    
    if AdminPresenceData.UI.SpectateBtn then
        if AdminPresenceData.Spectating and workspace.CurrentCamera.CameraSubject == (adminInfo.Player.Character and adminInfo.Player.Character:FindFirstChild("Humanoid")) then
            AdminPresenceData.UI.SpectateBtn.Text = "Unspectate"
            AdminPresenceData.UI.SpectateBtn.BackgroundColor3 = Library.Scheme.AccentColor
        else
            AdminPresenceData.UI.SpectateBtn.Text = "Spectate"
            AdminPresenceData.UI.SpectateBtn.BackgroundColor3 = Library.Scheme.MainColor
        end
    end
    
    if AdminPresenceData.ChatLogWidget and AdminPresenceData.ChatLogWidget.Visible then
        AdminPresenceData.UpdateChatLogsUI()
    end
end

function Library:InitAdminPresence(Config)
    if AdminPresenceData.Config then return end
    AdminPresenceData.Config = Config
    
    local function NewElement(ClassName, Props)
    ...
    local ScreenGui = NewElement("ScreenGui", {
        Name = "AdminPresenceGui",
        DisplayOrder = 997,
        ResetOnSpawn = false,
        Parent = Library.ScreenGui or gethui()
    })

    AdminPresenceData.Widget = NewElement("Frame", {
        Name = "AdminPresenceWidget", Size = UDim2.fromOffset(300, 160), Position = UDim2.new(0.5, -150, 0, -200),
        BackgroundColor3 = Library.Scheme.BackgroundColor, BorderSizePixel = 0, Parent = ScreenGui, ClipsDescendants = true, Visible = false
    })
    NewElement("UIStroke", { Color = Library.Scheme.OutlineColor, Thickness = 1, Parent = AdminPresenceData.Widget })
    NewElement("UICorner", { CornerRadius = UDim.new(0, 6), Parent = AdminPresenceData.Widget })
    
    local Topbar = NewElement("Frame", {
        Name = "Topbar", Size = UDim2.new(1, 0, 0, 24), BackgroundColor3 = Library.Scheme.MainColor, BorderSizePixel = 0, Parent = AdminPresenceData.Widget
    })
    NewElement("UICorner", { CornerRadius = UDim.new(0, 6), Parent = Topbar })
    ...
    NewElement("TextLabel", {
        Size = UDim2.new(1, -60, 1, 0), Position = UDim2.fromOffset(8, 0), BackgroundTransparency = 1,
        Text = "Admin Presence", TextColor3 = Library.Scheme.FontColor, FontFace = Library.Scheme.Font,
        TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left, Parent = Topbar
    })
    
    AdminPresenceData.UI.ToggleBtn = NewElement("TextButton", {
        Size = UDim2.fromOffset(24, 24), Position = UDim2.new(1, -24, 0, 0), BackgroundTransparency = 1,
        Text = "v", TextColor3 = Library.Scheme.FontColor, FontFace = Library.Scheme.Font, TextSize = 14, Parent = Topbar
    })
    
    AdminPresenceData.UI.SwitcherLeft = NewElement("TextButton", {
        Size = UDim2.fromOffset(20, 24), Position = UDim2.new(1, -94, 0, 0), BackgroundTransparency = 1,
        Text = "<", TextColor3 = Library.Scheme.FontColor, FontFace = Library.Scheme.Font, TextSize = 14, Parent = Topbar
    })
    
    AdminPresenceData.UI.SwitcherLabel = NewElement("TextLabel", {
        Size = UDim2.fromOffset(30, 24), Position = UDim2.new(1, -74, 0, 0), BackgroundTransparency = 1,
        Text = "1/1", TextColor3 = Library.Scheme.FontColor, FontFace = Library.Scheme.Font, TextSize = 12, Parent = Topbar
    })
    
    AdminPresenceData.UI.SwitcherRight = NewElement("TextButton", {
        Size = UDim2.fromOffset(20, 24), Position = UDim2.new(1, -44, 0, 0), BackgroundTransparency = 1,
        Text = ">", TextColor3 = Library.Scheme.FontColor, FontFace = Library.Scheme.Font, TextSize = 14, Parent = Topbar
    })
    
    Library:MakeDraggable(AdminPresenceData.Widget, Topbar)
    
    local ContentContainer = NewElement("Frame", {
        Size = UDim2.new(1, 0, 1, -24), Position = UDim2.fromOffset(0, 24), BackgroundTransparency = 1, Parent = AdminPresenceData.Widget
    })
    
    AdminPresenceData.UI.Avatar = NewElement("ImageLabel", {
        Size = UDim2.fromOffset(60, 60), Position = UDim2.fromOffset(10, 10), BackgroundColor3 = Library.Scheme.MainColor, Parent = ContentContainer
    })
    NewElement("UICorner", { CornerRadius = UDim.new(0, 6), Parent = AdminPresenceData.UI.Avatar })
    
    AdminPresenceData.UI.Name = NewElement("TextLabel", {
        Size = UDim2.new(1, -90, 0, 16), Position = UDim2.fromOffset(80, 10), BackgroundTransparency = 1,
        Text = "Name", TextColor3 = Library.Scheme.FontColor, FontFace = Library.Scheme.Font, TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left, Parent = ContentContainer
    })
    
    AdminPresenceData.UI.Role = NewElement("TextLabel", {
        Size = UDim2.new(1, -90, 0, 14), Position = UDim2.fromOffset(80, 28), BackgroundTransparency = 1,
        Text = "Role", TextColor3 = Library.Scheme.AccentColor, FontFace = Library.Scheme.Font, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left, Parent = ContentContainer
    })
    
    AdminPresenceData.UI.JoinTime = NewElement("TextLabel", {
        Size = UDim2.new(1, -90, 0, 14), Position = UDim2.fromOffset(80, 44), BackgroundTransparency = 1,
        Text = "Joined: 00:00", TextColor3 = Color3.fromRGB(180, 180, 180), FontFace = Library.Scheme.Font, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left, Parent = ContentContainer
    })
    
    AdminPresenceData.UI.Team = NewElement("TextLabel", {
        Size = UDim2.new(1, -90, 0, 14), Position = UDim2.fromOffset(80, 58), BackgroundTransparency = 1,
        Text = "Team: —", TextColor3 = Color3.fromRGB(160, 160, 255), FontFace = Library.Scheme.Font, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left, Parent = ContentContainer
    })
    
    AdminPresenceData.UI.LastAction = NewElement("TextLabel", {
        Size = UDim2.new(1, -20, 0, 14), Position = UDim2.fromOffset(10, 80), BackgroundTransparency = 1,
        Text = "Last Action: Joined", TextColor3 = Color3.fromRGB(200, 200, 200), FontFace = Library.Scheme.Font, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left, Parent = ContentContainer
    })
    
    local ButtonsContainer = NewElement("Frame", {
        Size = UDim2.new(1, -20, 0, 24), Position = UDim2.new(0, 10, 1, -34), BackgroundTransparency = 1, Parent = ContentContainer
    })
    NewElement("UIListLayout", { Parent = ButtonsContainer, FillDirection = Enum.FillDirection.Horizontal, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 5) })
    
    AdminPresenceData.UI.SpectateBtn = NewElement("TextButton", {
        Size = UDim2.new(0.33, -3, 1, 0), BackgroundColor3 = Library.Scheme.MainColor, Text = "Spectate", TextColor3 = Library.Scheme.FontColor, FontFace = Library.Scheme.Font, TextSize = 12, Parent = ButtonsContainer
    })
    NewElement("UICorner", { CornerRadius = UDim.new(0, 4), Parent = AdminPresenceData.UI.SpectateBtn })
    NewElement("UIStroke", { Color = Library.Scheme.OutlineColor, Parent = AdminPresenceData.UI.SpectateBtn })
    
    AdminPresenceData.UI.TeleportBtn = NewElement("TextButton", {
        Size = UDim2.new(0.33, -4, 1, 0), BackgroundColor3 = Library.Scheme.MainColor, Text = "Teleport", TextColor3 = Library.Scheme.FontColor, FontFace = Library.Scheme.Font, TextSize = 12, Parent = ButtonsContainer
    })
    NewElement("UICorner", { CornerRadius = UDim.new(0, 4), Parent = AdminPresenceData.UI.TeleportBtn })
    NewElement("UIStroke", { Color = Library.Scheme.OutlineColor, Parent = AdminPresenceData.UI.TeleportBtn })
    
    AdminPresenceData.UI.ChatLogsBtn = NewElement("TextButton", {
        Size = UDim2.new(0.33, -3, 1, 0), BackgroundColor3 = Library.Scheme.MainColor, Text = "Chat Logs", TextColor3 = Library.Scheme.FontColor, FontFace = Library.Scheme.Font, TextSize = 12, Parent = ButtonsContainer
    })
    NewElement("UICorner", { CornerRadius = UDim.new(0, 4), Parent = AdminPresenceData.UI.ChatLogsBtn })
    NewElement("UIStroke", { Color = Library.Scheme.OutlineColor, Parent = AdminPresenceData.UI.ChatLogsBtn })

    -- Chat Logs UI
    AdminPresenceData.ChatLogWidget = NewElement("Frame", {
        Name = "AdminChatLogsWidget", Size = UDim2.fromOffset(300, 200), Position = UDim2.new(0.5, -150, 0.5, -100),
        BackgroundColor3 = Library.Scheme.BackgroundColor, BorderSizePixel = 0, Parent = ScreenGui, Visible = false
    })
    NewElement("UIStroke", { Color = Library.Scheme.OutlineColor, Thickness = 1, Parent = AdminPresenceData.ChatLogWidget })
    NewElement("UICorner", { CornerRadius = UDim.new(0, 6), Parent = AdminPresenceData.ChatLogWidget })
    
    local ChatTopbar = NewElement("Frame", {
        Name = "ChatTopbar", Size = UDim2.new(1, 0, 0, 24), BackgroundColor3 = Library.Scheme.MainColor, BorderSizePixel = 0, Parent = AdminPresenceData.ChatLogWidget
    })
    NewElement("UICorner", { CornerRadius = UDim.new(0, 6), Parent = ChatTopbar })
    NewElement("Frame", { Size = UDim2.new(1, 0, 0, 6), Position = UDim2.new(0,0,1,-6), BackgroundColor3 = Library.Scheme.MainColor, BorderSizePixel = 0, Parent = ChatTopbar })
    
    AdminPresenceData.UI.ChatLogsTitle = NewElement("TextLabel", {
        Size = UDim2.new(1, -30, 1, 0), Position = UDim2.fromOffset(8, 0), BackgroundTransparency = 1,
        Text = "Chat Logs", TextColor3 = Library.Scheme.FontColor, FontFace = Library.Scheme.Font, TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left, Parent = ChatTopbar
    })
    
    AdminPresenceData.UI.ChatLogsClose = NewElement("TextButton", {
        Size = UDim2.fromOffset(24, 24), Position = UDim2.new(1, -24, 0, 0), BackgroundTransparency = 1,
        Text = "X", TextColor3 = Library.Scheme.RedColor or Color3.fromRGB(255, 50, 50), FontFace = Library.Scheme.Font, TextSize = 14, Parent = ChatTopbar
    })
    
    Library:MakeDraggable(AdminPresenceData.ChatLogWidget, ChatTopbar)
    
    AdminPresenceData.ChatLogsContainer = NewElement("ScrollingFrame", {
        Size = UDim2.new(1, -16, 1, -32), Position = UDim2.fromOffset(8, 28), BackgroundTransparency = 1, BorderSizePixel = 0,
        ScrollBarThickness = 2, CanvasSize = UDim2.new(0,0,0,0), Parent = AdminPresenceData.ChatLogWidget
    })
    NewElement("UIListLayout", {
        Parent = AdminPresenceData.ChatLogsContainer, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 2)
    })

    -- Buttons Logic
    AdminPresenceData.UI.SwitcherLeft.MouseButton1Click:Connect(function()
        if AdminPresenceData.CurrentIndex > 1 then
            AdminPresenceData.CurrentIndex = AdminPresenceData.CurrentIndex - 1
            AdminPresenceData.UpdateUI()
        end
    end)
    
    AdminPresenceData.UI.SwitcherRight.MouseButton1Click:Connect(function()
        if AdminPresenceData.CurrentIndex < #AdminPresenceData.Admins then
            AdminPresenceData.CurrentIndex = AdminPresenceData.CurrentIndex + 1
            AdminPresenceData.UpdateUI()
        end
    end)
    
    AdminPresenceData.UI.ToggleBtn.MouseButton1Click:Connect(function()
        AdminPresenceData.Collapsed = not AdminPresenceData.Collapsed
        if AdminPresenceData.Collapsed then
            AdminPresenceData.UI.ToggleBtn.Text = "^"
            ContentContainer.Visible = false
            TweenService:Create(AdminPresenceData.Widget, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Size = UDim2.fromOffset(300, 24)
            }):Play()
        else
            AdminPresenceData.UI.ToggleBtn.Text = "v"
    ...
    AdminPresenceData.UI.ChatLogsClose.MouseButton1Click:Connect(function()
        AdminPresenceData.ChatLogWidget.Visible = false
    end)
    
    AdminPresenceData.UI.ChatLogsBtn.MouseButton1Click:Connect(function()
        AdminPresenceData.ChatLogWidget.Visible = true
        AdminPresenceData.UpdateChatLogsUI()
    end)
    
    AdminPresenceData.UI.SpectateBtn.MouseButton1Click:Connect(function()
        local adminInfo = AdminPresenceData.Admins[AdminPresenceData.CurrentIndex]
        if not adminInfo then return end
        
        if AdminPresenceData.Spectating and workspace.CurrentCamera.CameraSubject == (adminInfo.Player.Character and adminInfo.Player.Character:FindFirstChild("Humanoid")) then
            workspace.CurrentCamera.CameraSubject = LocalPlayer.Character:FindFirstChild("Humanoid")
            AdminPresenceData.Spectating = false
        else
            if adminInfo.Player.Character and adminInfo.Player.Character:FindFirstChild("Humanoid") then
                workspace.CurrentCamera.CameraSubject = adminInfo.Player.Character.Humanoid
                AdminPresenceData.Spectating = true
            end
        end
        AdminPresenceData.UpdateUI()
    end)
    
    local TeleportConfirming = false
    AdminPresenceData.UI.TeleportBtn.MouseButton1Click:Connect(function()
        if not TeleportConfirming then
            TeleportConfirming = true
            AdminPresenceData.UI.TeleportBtn.Text = "Yes?"
            AdminPresenceData.UI.TeleportBtn.BackgroundColor3 = Color3.fromRGB(45, 204, 112)
            task.delay(3, function()
                if TeleportConfirming then
                    TeleportConfirming = false
                    AdminPresenceData.UpdateUI()
                end
            end)
        else
            TeleportConfirming = false
            local adminInfo = AdminPresenceData.Admins[AdminPresenceData.CurrentIndex]
            if adminInfo and adminInfo.Player.Character and adminInfo.Player.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                LocalPlayer.Character.HumanoidRootPart.CFrame = adminInfo.Player.Character.HumanoidRootPart.CFrame
            end
            AdminPresenceData.UpdateUI()
        end
    end)
    
    -- Player connection tracking logic
    local function HandleAction(player, actionText)
        for _, admin in ipairs(AdminPresenceData.Admins) do
            if admin.Player == player then
                admin.LastAction = actionText
                if AdminPresenceData.Admins[AdminPresenceData.CurrentIndex] == admin then
                    AdminPresenceData.UpdateUI()
                end
                break
            end
    ...
    local function CheckPlayer(player)
        task.spawn(function()
            local success, rank = pcall(function() return player:GetRankInGroup(Config.GroupId) end)
            if success and Config.StaffRanks[rank] then
                local joinTime = os.date("%H:%M:%S")
                local adminInfo = {
                    Player = player,
                    Rank = rank,
                    Role = Config.StaffRanks[rank],
                    JoinTime = joinTime,
                    LastAction = "Joined server",
                    Logs = {}
                }
                table.insert(AdminPresenceData.Admins, adminInfo)
                AdminPresenceData.UpdateUI()
                ...
                Library:Notify({
                    ...
                    Description = string.format("%s (%s) joined the server — %s", player.DisplayName, player.Name, Config.StaffRanks[rank]),
                    Time = 6,
                })
                
                local conn1 = player.Chatted:Connect(function(msg)
                    table.insert(adminInfo.Logs, string.format("[%s]: %s", os.date("%H:%M:%S"), msg))
                    HandleAction(player, "Chatted")
                    if AdminPresenceData.ChatLogWidget.Visible and AdminPresenceData.Admins[AdminPresenceData.CurrentIndex] == adminInfo then
                        AdminPresenceData.UpdateChatLogsUI()
                    end
                end)
                
                local conn2 = player:GetPropertyChangedSignal("Team"):Connect(function()
                    adminInfo.LastAction = "Changed team"
                    if AdminPresenceData.Admins[AdminPresenceData.CurrentIndex] == adminInfo then
                        AdminPresenceData.UpdateUI()
                    end
                end)
                
                AdminPresenceData.Connections[player.UserId] = {conn1, conn2}
            end
        end)
    end
    
    Library:GiveSignal(Players.PlayerAdded:Connect(CheckPlayer))
    for _, player in ipairs(Players:GetPlayers()) do
        CheckPlayer(player)
    end
    
    Library:GiveSignal(Players.PlayerRemoving:Connect(function(player)
        if AdminPresenceData.Connections[player.UserId] then
            for _, conn in ipairs(AdminPresenceData.Connections[player.UserId]) do conn:Disconnect() end
            AdminPresenceData.Connections[player.UserId] = nil
        end
        
        for i, admin in ipairs(AdminPresenceData.Admins) do
            if admin.Player == player then
                table.remove(AdminPresenceData.Admins, i)
                AdminPresenceData.UpdateUI()
                break
            end
        end
...
function Library:CreateChatLogs(Config)
    ...
    local ChatLogsData = {
        Messages = {},
        MaxMessages = Config.MaxMessages or 500,
        Container = nil,
        ScrollingFrame = nil,
        UIListLayout = nil,
        IsAtBottom = true,
    ...
    ChatLogsData.Widget = NewElement("Frame", {
        Name = "ChatLogsWidget",
    ...
    local Title = NewElement("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = Config.Title or "Chat Logs",
    ...
    NewElement("UICorner", { CornerRadius = UDim.new(0, 4), Parent = ClearBtn })
    ...
    NewElement("UICorner", { CornerRadius = UDim.new(0, 4), Parent = ToggleBtn })
    ...
    ChatLogsData.ScrollingFrame = NewElement("ScrollingFrame", {
    ...
    local function UpdateCanvasSize()
        if ChatLogsData.UIListLayout then
            ChatLogsData.ScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, ChatLogsData.UIListLayout.AbsoluteContentSize.Y + 10)
            if ChatLogsData.IsAtBottom then
                ChatLogsData.ScrollingFrame.CanvasPosition = Vector2.new(0, ChatLogsData.ScrollingFrame.CanvasSize.Y.Offset)
            end
        end
    end

    ChatLogsData.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(UpdateCanvasSize)

    -- Scroll detection
    ChatLogsData.ScrollingFrame:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
        local canvasPos = ChatLogsData.ScrollingFrame.CanvasPosition.Y
        local canvasSize = ChatLogsData.ScrollingFrame.CanvasSize.Y.Offset
        local viewportSize = ChatLogsData.ScrollingFrame.AbsoluteWindowSize.Y
        
        -- Check if user is at bottom (with small threshold)
        ChatLogsData.IsAtBottom = (canvasPos >= canvasSize - viewportSize - 5)
    end)

    -- Add message function
    local function AddMessage(playerName, playerDisplayName, message, teamColor, timestamp)
        timestamp = timestamp or os.date("%H:%M:%S")
        teamColor = teamColor or Library.Scheme.FontColor
        ...
        if msgCount > ChatLogsData.MaxMessages then
            local oldestMsg = nil
            local oldestLayoutOrder = math.huge
            for _, child in ipairs(children) do
                if child:IsA("TextLabel") and child.LayoutOrder < oldestLayoutOrder then
                    oldestLayoutOrder = child.LayoutOrder
                    oldestMsg = child
                end
            end
            if oldestMsg then oldestMsg:Destroy() end
        end
    ...
    ClearBtn.MouseButton1Click:Connect(function()
        for _, child in ipairs(ChatLogsData.ScrollingFrame:GetChildren()) do
            if child:IsA("TextLabel") then child:Destroy() end
        end
        ChatLogsData.Messages = {}
        UpdateCanvasSize()
    end)
    ...
    ToggleBtn.MouseButton1Click:Connect(function()
        ...
        if collapsed then
            ToggleBtn.Text = "^"
            ContentContainer.Visible = false
            TweenService:Create(ChatLogsData.Widget, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Size = UDim2.fromOffset(Config.Width or 500, 28)
            }):Play()
        else
            ToggleBtn.Text = "v"
            ContentContainer.Visible = true
    ...
    CloseBtn.MouseButton1Click:Connect(function()
        ChatLogsData.Widget.Visible = false
    end)

    -- Public API
    local WidgetAPI = {}
    
    function WidgetAPI:AddMessage(playerName, message, teamColor, timestamp)
        AddMessage(playerName, playerName, message, teamColor, timestamp)
    end

    function WidgetAPI:AddPlayerMessage(player, message, timestamp)
        local teamColor = Library.Scheme.FontColor
        if player.Team and player.Team.TeamColor then
            teamColor = player.Team.TeamColor.Color
        end
        AddMessage(player.Name, player.DisplayName, message, teamColor, timestamp)
    end

    function WidgetAPI:Clear()
        for _, child in ipairs(ChatLogsData.ScrollingFrame:GetChildren()) do
            if child:IsA("TextLabel") then child:Destroy() end
        end
        ChatLogsData.Messages = {}
    ...
    function WidgetAPI:SetVisible(visible)
        ChatLogsData.Widget.Visible = visible
    end

    function WidgetAPI:IsVisible()
        return ChatLogsData.Widget.Visible
    end

    function WidgetAPI:SetPosition(position)
        ChatLogsData.Widget.Position = position
    end

    function WidgetAPI:GetFrame()
        return ChatLogsData.Widget
    end

    function WidgetAPI:Destroy()
        for _, conn in ipairs(ChatLogsData.Connections) do
            conn:Disconnect()
        end
        ScreenGui:Destroy()
    end

    -- Hook into Player.Chatted if configured
    if Config.AutoCapture ~= false then
        local function onPlayerChatted(player, message)
            WidgetAPI:AddPlayerMessage(player, message)
        end

        for _, player in ipairs(Players:GetPlayers()) do
            table.insert(ChatLogsData.Connections, player.Chatted:Connect(function(msg) onPlayerChatted(player, msg) end))
        end
        table.insert(ChatLogsData.Connections, Players.PlayerAdded:Connect(function(player)
            table.insert(ChatLogsData.Connections, player.Chatted:Connect(function(msg) onPlayerChatted(player, msg) end))
        end))
    end
